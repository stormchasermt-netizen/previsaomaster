...
// Replace fetchIpmetImage
const newFetch = `export async function fetchIpmetImage(nominalTs12: string): Promise<{ buffer: Buffer; ts12: string } | null> {
  let trueTs12 = '';
  try {
...
const regex = /export async function fetchIpmetImage\(nominalTs12: string\): Promise<\{ buffer: Buffer; ts12: string \} \| null> \{[\s\S]*?\}/;
code = code.replace(regex, newFetch);

fs.writeFileSync('cloud-run/radar-ao-vivo2-feeder/src/radarFetch.ts', code);
console.log("Replaced fetchIpmetImage");
