...
# --- CONFIGURAÇÕES ORIGINAIS DO VITOR (CENTRO-SUL) ---
skip = 35      # Ventos originais (bem espaçados)
hodo_step = 40 # Hodógrafos originais
...
def plot_background(cart_proj, lons, lats):
    ...
    # --- AS MARGENS ORIGINAIS DO VITOR ---
    ax.set_extent([lons.min() + 0.5, lons.max() - 0.6, lats.min() + 0.2, lats.max()], crs=ccrs.PlateCarree())
...
def process_file(file_path):
    try:
        ...
        # --- VOLTANDO PARA A PASTA ORIGINAL ---
        init_dir = init.strftime('%Y%m%d_%H%M%S')
...
if __name__ == "__main__":
    ...
    print(f"==================================================")
    print(f" RESTAURANDO IMAGENS ORIGINAIS (CENTRO-SUL) ")
    print(f" Processadores: {num_cpus} | Arquivos: {len(files)}")
    print(f"==================================================")
