...
const AUTO_JOB_INTERVAL_MS = Math.max(60_000, parseInt(process.env.AUTO_JOB_INTERVAL_MS || String(5 * 60 * 1000), 10) || 5 * 60 * 1000);
...
async function saveIfNotExists(bucket, slug, fileName, buffer, sourceUrl) {
...
async function executeSync(targetSlug) {
...
async function executeCleanup() {
    ...
    for (const slug of SYNC_SLUGS) {
        ...
        const fileData = files.map(f => {
            ...
            if (!m)
                return { f, valid: false, t: 0, isDoppler: false };
            const ts12 = m[1];
        ...
        const ppiRetained = new Set();
        const dopRetained = new Set();
        let ppiCount = 0;
        let dopCount = 0;
...
async function runAutoCycle() {
    ...
    try {
        ...
        console.log(`[auto] Sync OK nominalTs12=${syncResult.nominalTs12} okCount=${syncResult.okCount} failCount=${syncResult.failCount}`);
...
app.all('/sync', requireSecret, async (req, res) => {
    try {
        const targetSlug = req.query.slug;
...
app.listen(PORT, () => {
    ...
    console.log(`Sync window: ${SYNC_WINDOW_MINUTES} min, retention ${RETENTION_MINUTES} min`);
    console.log(`Slugs (${SYNC_SLUGS.length}):`, SYNC_SLUGS.join(', '));
    if (ENABLE_AUTO_JOBS) {
        console.log(`[auto] Jobs automáticos ON — intervalo ${AUTO_JOB_INTERVAL_MS} ms, 1.º ciclo após ${AUTO_JOB_START_DELAY_MS} ms`);
        setTimeout(() => {
            void runAutoCycle();
