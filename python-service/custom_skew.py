...
class backgroundSkewT(QWidget):
    ...
    def initUI(self):
        ...
        if self.physicalDpiX() > 75:
            ...
            fsize = 7
            fsizet = 14
        self.title_font = QtGui.QFont('Helvetica', int(fsizet + (self.hgt * 0.006)))
        self.title_metrics = QtGui.QFontMetrics( self.title_font )
        #self.title_font.setBold(True)
        self.title_height = self.title_metrics.xHeight() + 15 + (self.hgt * 0.003)

        self.label_font = QtGui.QFont('Helvetica', int(fsize + 2 + (self.hgt * 0.0045)))
        self.environment_trace_font = QtGui.QFont('Helvetica', int(11 + (self.hgt * 0.0045)))
        self.in_plot_font = QtGui.QFont('Helvetica', int(fsize + (self.hgt * 0.0045)))
        self.esrh_font = QtGui.QFont('Helvetica', int(fsize + 2 + (self.hgt * 0.0045)))
        self.hght_font = QtGui.QFont('Helvetica', int(9 + (self.hgt * 0.0045)))
...
#       print bg_rgb, self.fg_color.getRgb()
        ...
        #ag = QtGui.QActionGroup(self); ag.setExclusive(True)
        ...
        ag = QtWidgets.QActionGroup(self); ag.setExclusive(True)
...
#           idx_pbot, idx_ptop = np.searchsorted(np.array([self.prof.dgz_pbot, self.prof.dgz_ptop]), self.prof.pres)
    ...
    def drawTitles(self, qp):
        ...
        rect0 = QtCore.QRect(int(self.lpad), int(2), int(box_width), int(self.title_height))
