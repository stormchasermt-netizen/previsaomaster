...
import { User, Users, MapPin, BookOpen, HelpCircle, X, Shield, Zap, TrendingUp, Medal, Hash, Gamepad2, ChevronLeft, ArrowRight, UserPlus, Check, MessageSquare } from 'lucide-react';
...
export default function JogosPage() {
      ...
      const loadPlayers = async () => {
        ...
        scores.forEach(s => {
          if (!playersMap.has(s.userId)) {
            playersMap.set(s.userId, { uid: s.userId, displayName: s.displayName, photoURL: s.photoURL, isOnline: false });
          }
        });
  ...
  const handleSendInvite = async (uid: string) => {
    ...
    let code = lobby?.code;
    if (!code) {
      try { code = await createLobby('iniciante'); } catch (e) { console.error("Failed to auto-create lobby for invite", e); }
    }
    if (code) await sendInvite(uid, code);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[80vh] animate-in fade-in duration-700 relative">
      <div className="text-center mb-12">
        <Gamepad2 className="w-16 h-16 text-cyan-400 mx-auto mb-4" strokeWidth={1.5} />
        <h1 className="text-4xl md:text-5xl font-black text-white tracking-wide uppercase mb-2">Jogos</h1>
        <p className="text-slate-400 text-base">Pratique e desafie outros previsores</p>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-6 mb-12">
        ...
        <Link href="/previsao" className="group block">
          <div className="w-64 h-40 bg-[#161b22] border border-white/10 rounded-lg flex flex-col items-center justify-center hover:border-amber-500/50 hover:bg-[#1c2128] transition-all shadow-lg relative overflow-hidden">
            ...
            <h3 className="text-white font-bold group-hover:text-amber-400 transition-colors">Previsao & Sondagens</h3>
            <p className="text-[10px] text-slate-500 mt-1 uppercase tracking-wider group-hover:text-slate-400">Desenhe poligonos, ganhe pontos</p>
          </div>
        </Link>
      </div>

      <Link href="/" className="flex items-center gap-2 text-slate-400 hover:text-white text-sm transition-colors">
        <ChevronLeft className="w-4 h-4" /> Voltar ao menu principal
      </Link>
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-2">
            {allKnownPlayers.length === 0 && (
              <div className="text-center p-8 text-slate-500 text-sm">
                ...
                Nenhum jogador encontrado no historico.
              </div>
            )}
            {allKnownPlayers.map(p => (
              <div key={p.uid} className="bg-slate-900 border border-white/5 p-3 rounded-lg flex items-center gap-3 group hover:border-white/20 transition-all">
                <div className="w-8 h-8 rounded-full bg-slate-800 overflow-hidden shrink-0 relative">
                  {p.photoURL ? <img src={p.photoURL} className="w-full h-full object-cover" alt="" /> : <div className="flex items-center justify-center h-full font-bold text-slate-500">{p.displayName[0]}</div>}
                  <div className={clsx("absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border border-slate-900", p.isOnline ? "bg-emerald-500" : "bg-slate-500")} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="font-bold text-white truncate text-sm">{p.displayName}</div>
                  <div className="text-[10px] text-slate-500 flex items-center gap-1">
                    <span className={clsx("w-1.5 h-1.5 rounded-full", p.isOnline ? "bg-emerald-500" : "bg-slate-600")} />
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-slate-300 text-xs mt-1">
                <span className="text-white font-bold">{incomingInvite.hostName}</span> convidou voce para uma partida.
              </p>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in">
          <div className="bg-slate-900 border border-white/10 rounded-2xl p-6 w-full max-w-md shadow-2xl relative overflow-hidden">
              ...
              <div className="animate-in slide-in-from-right duration-300">
                ...
                <div className="space-y-4">
                  <button onClick={() => setMultiplayerView('create')} className="w-full bg-cyan-600 hover:bg-cyan-500 text-white p-4 rounded-xl flex items-center justify-between group transition-all">
                    <div className="flex items-center gap-3"><div className="bg-black/20 p-2 rounded-lg"><Gamepad2 className="w-6 h-6" /></div><div className="text-left"><div className="font-bold">Criar Nova Sala</div><div className="text-xs text-cyan-200">Voce sera o Host</div></div></div>
                  ...
                  <button onClick={() => setMultiplayerView('join')} className="w-full bg-slate-800 hover:bg-slate-700 text-white p-4 rounded-xl flex items-center justify-between group border border-white/10 transition-all">
                    <div className="flex items-center gap-3"><div className="bg-black/20 p-2 rounded-lg"><Hash className="w-6 h-6 text-slate-300" /></div><div className="text-left"><div className="font-bold">Entrar com Codigo</div><div className="text-xs text-slate-400">Junte-se a uma sala existente</div></div></div>
                    <ArrowRight className="w-5 h-5 opacity-50 group-hover:opacity-100 group-hover:translate-x-1 transition-all" />
                  </button>
                </div>
              </div>
              ...
              <div className="animate-in slide-in-from-right duration-300">
                ...
                <div className="space-y-3 mb-8">
                  {difficulties.map((diff) => (
                    <button key={diff.id} onClick={() => setSelectedDifficulty(diff.id as PrevisaoDifficulty)} className={clsx("w-full flex items-center p-3 rounded-lg border transition-all text-left group", selectedDifficulty === diff.id ? `${diff.bg} ${diff.border} shadow-[0_0_15px_rgba(0,0,0,0.5)] scale-[1.02]` : "bg-slate-800/50 border-transparent hover:bg-slate-800 hover:border-white/10")}>
                      <div className={clsx("p-2 rounded-md mr-3 bg-black/20", diff.color)}><diff.icon className="w-5 h-5" /></div>
                      <div><div className={clsx("font-bold text-sm", selectedDifficulty === diff.id ? "text-white" : "text-slate-300")}>{diff.label}</div><div className="text-xs text-slate-500">{diff.desc}</div></div>
                      {selectedDifficulty === diff.id && (<div className="ml-auto w-2 h-2 rounded-full bg-cyan-400 shadow-[0_0_5px_cyan]" />)}
                    </button>
                  ))}
                </div>
            ...
            {multiplayerView === 'join' && (
              <div className="animate-in slide-in-from-right duration-300">
                <button onClick={() => setMultiplayerView('menu')} className="absolute top-4 left-4 text-slate-500 hover:text-white flex items-center gap-1 text-xs font-bold uppercase tracking-wider"><ChevronLeft className="w-4 h-4" /> Voltar</button>
                <div className="text-center mb-8 mt-4"><Hash className="w-12 h-12 text-emerald-400 mx-auto mb-3" /><h2 className="text-xl font-bold text-white uppercase">Digitar Codigo</h2></div>
                <div className="space-y-6">
                  <div>
                    <input value={joinCode} onChange={(e) => setJoinCode(e.target.value.toUpperCase())} placeholder="CODIGO" maxLength={8} className="w-full bg-slate-950 border-2 border-slate-700 focus:border-emerald-500 rounded-xl px-4 py-4 text-center text-3xl font-black text-white tracking-[0.5em] placeholder:tracking-normal placeholder:text-slate-700 outline-none transition-colors" />
                    <p className="text-center text-xs text-slate-500 mt-2">Exemplo: X7K9P2</p>
                  </div>
                  <button onClick={handleJoinLobby} disabled={joinCode.length < 4} className="w-full bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold py-3 rounded-xl shadow-lg shadow-emerald-900/40 transition-all hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-2">Entrar na Sala <ArrowRight className="w-4 h-4" /></button>
                </div>
              </div>
            )}
          </div>
