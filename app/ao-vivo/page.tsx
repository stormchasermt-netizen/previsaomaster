...
export default function AoVivoPage() {
  ...
  // Memória global das imagens já processadas (URL do Blob local)
  const preloadedFramesRef = useRef<Map<string, string>>(new Map());
  // Controle de quais frames já mandamos carregar para não repetir
  ...
  /** MOTOR DE PRELOAD SILENCIOSO (O SEGREDO DO ZERO FLICKER) */
  useEffect(() => {
    if (!mapReady || !mapInstanceRef.current || !validSliderMinutesAgo || validSliderMinutesAgo.length === 0 || displayRadars.length === 0) return;

    const map = mapInstanceRef.current;
    const baseTs12 = historicalTimestampOverride || getNowMinusMinutesTimestamp12UTC(3);
    const isLive = !historicalTimestampOverride;
    const product = splitCount >= 2 ? 'reflectividade' : radarProductType;

    // Roda em todos os frames de tempo válidos da última hora
    validSliderMinutesAgo.forEach((minutesAgo) => {
      displayRadars.forEach((dr) => {
        ...
        const frameKey = `${radarKey}-${product}-${exactTs12}`; // Chave única deste frame

        // Se já baixamos ou já estamos baixando, ignora
        ...
        const apiUrl = `/api/radar-frame?slug=${slugParam}&ts12=${exactTs12}&product=${product}&isLive=${isLive}&sourceMode=${radarSourceMode}`;

        // 1. Pede a URL pra API
        fetch(apiUrl)
          .then(r => r.ok ? r.json() : Promise.reject())
          .then(data => {
            if (!data?.url) return;

            // 2. Manda pro Web Worker limpar o ruído (em background)
            if (radarWorkerRef.current) {
              ...
              workerCallbacks.current.set(reqId, (processedUrl, err) => {
                if (!err && processedUrl) {
                  ...
                  // 3. INJETA NO MAPA INVISÍVEL (opacity: 0)
                  const sourceId = `src-${frameKey}`;
                  const layerId = `lyr-${frameKey}`;
                  const bounds = getBoundsForDisplayRadar(dr, data.source);
                  const coords: [[number,number],[number,number],[number,number],[number,number]] = [[bounds.west, bounds.north], [bounds.east, bounds.north],
                  ...
                  if (!map.getSource(sourceId)) {
                    ...
                    map.addLayer({
                      ...
                      source: sourceId,
                      paint: {
                        'raster-opacity': initialOpacity, // NASCE INVISÍVEL ou visivel se for o frame atual
                        'raster-fade-duration': 400 // TRANSICAO SUAVE DE 400ms NATIVA DA GPU!
                      }
                    });
                  }
                  
                  // Atualiza as sources para a UI mostrar os ícones corretos de disponibilidade
                  setRadarEffectiveTimestamps((prev) => ({ ...prev, [radarKey]: exactTs12 }));
                  setRadarEffectiveSource((prev) => ({ ...prev, [radarKey]: data.source }));
              ...
              radarWorkerRef.current.postMessage({
                id: reqId, imageUrl: getProxiedRadarUrl(data.url), type: dr.type === 'cptec' ? dr.station.slug : 'argentina', product
              });
            }
          }).catch(() => { 
  ...
  },[validSliderMinutesAgo, displayRadars, radarProductType, splitCount, radarSourceMode, historicalTimestampOverride, mapReady, getBoundsForDisplayRadar]);
