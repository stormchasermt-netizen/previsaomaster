...
if (baseMapId === 'dark') {
  ...
  /** Bounds calculados a partir do centro e raio. Preserva bounds originais (ex: retangulares do mosaico IPMet). */
  const bounds = React.useMemo(() => {
    const s = selectedStation?.station as CptecRadarStation;
    if (s && s.bounds) {
        // Se a estação tem bounds originais definidos (ex: IPMet), preservamos a proporção e aplicamos shift/scale
        const offsetLat = (imageCenterLat !== 0 ? imageCenterLat : centerLat) - s.lat;
        const offsetLng = (imageCenterLng !== 0 ? imageCenterLng : centerLng) - s.lng;
        const scale = s.rangeKm ? (rangeKm / s.rangeKm) : 1;
        
        const origCenterLat = (s.bounds.maxLat + s.bounds.minLat) / 2;
        const origCenterLng = (s.bounds.maxLon + s.bounds.minLon) / 2;
        
        const height = (s.bounds.maxLat - s.bounds.minLat) * scale;
        const width = (s.bounds.maxLon - s.bounds.minLon) * scale;
        
        const newCenterLat = origCenterLat + offsetLat;
        const newCenterLng = origCenterLng + offsetLng;
        
        return {
            ne: { lat: newCenterLat + height / 2, lng: newCenterLng + width / 2 },
            sw: { lat: newCenterLat - height / 2, lng: newCenterLng - width / 2 }
        };
    }

    const isIpmet = selectedStation?.type === 'cptec' && 
      (s?.slug === 'ipmet-bauru' || s?.slug === 'ipmet-prudente');
  ...
  const handleSaveImagePosition = useCallback(async (newImgLat: number, newImgLng: number) => {
    ...
    /** Salva posição (lat/lng) após arrastar o Centro do Radar. Move a imagem junto para preservar a posição relativa. */
