...
import { RADAR_V2_COORDS } from '@/lib/radarV2Coords';
...
export default function AoVivoPage() {
  ...
  const activeFrameOverlaysRef = useRef<any[]>([]);
  const activeFrameOverlays2Ref = useRef<any[]>([]);
  const activeFrameOverlays3Ref = useRef<any[]>([]);
  const activeFrameOverlays4Ref = useRef<any[]>([]);
  ...
  }, [mapReady, map2Ready, handleSampleAll, showCrosshair]);
  ...
  const getBoundsForDisplayRadar = useCallback(
    (dr: DisplayRadar) => {
      const radarKey = dr.type === 'cptec' ? dr.station.slug : `argentina:${dr.station.id}`;
      
      // PRIORIDADE 0: Tabela técnica de alta precisão V2 (CPTEC)
      if (dr.type === 'cptec' && RADAR_V2_COORDS[dr.station.slug]) {
        const v2 = RADAR_V2_COORDS[dr.station.slug];
        return v2.bounds;
      }
